...
void ScraperDialog::setupUi()
{
    ...
    m_platformCombo->addItems({"Auto-detect", "YouTube", "TikTok", "Instagram"});
    ...
    m_urlInput->setPlaceholderText(
        ...
        "when 'Username' type is selected");
...
void ScraperDialog::onScrape()
{
    ...
    // Build job — use first URL for now (multi-URL support later)
    ScrapeJob job;
    job.sourceUrl  = urls.first();
    job.maxVideos  = m_limitSpin->value();
    job.platform   = m_platformCombo->currentText() == "Auto-detect"
                     ? ScraperEngine::detectPlatform(urls.first())
...
void ScraperDialog::onFinished(int total, bool aborted)
{
    ...
    if (total == 0 && !aborted) {
        m_statusLbl->setText("No videos found. Check the URL and try again.");
    ...
    m_statusLbl->setText(
        aborted
        ? QString("Stopped — %1 video(s) found.").arg(total)
        : QString("Done! Found %1 video(s). Select and click Download.").arg(total));
