...
static bool isChannelOrProfileUrl(const QString &url)
{
    ...
    // Instagram profile
    if (QRegularExpression(R"(^https?://(?:www\.)?instagram\.com/([^/?]+)/?$)").match(url).hasMatch()
        && !url.contains("/p/") && !url.contains("/reel/") && !url.contains("/reels/")
        && !url.contains("/stories/") && !url.contains("/explore"))             return true;
    return false;
}
