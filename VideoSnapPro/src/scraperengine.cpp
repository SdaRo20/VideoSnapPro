...
bool ScraperEngine::isProfileUrl(const QString &url)
{
    ...
    if (url.contains("youtube.com/channel/")) return true;
    ...
    if (ttRe.match(url).hasMatch())           return true;
    static QRegularExpression igRe(R"(instagram\.com/([^/?\s]+)/?$)");
    auto igM = igRe.match(url);
    if (igM.hasMatch()) {
        QString seg = igM.captured(1);
        if (seg != "p" && seg != "reel" && seg != "reels" &&
            seg != "explore" && seg != "stories")
            return true;
    }
    return false;
}
...
{
    if (!m_proc) return;
    // Discard stderr — don't let it cause issues
