...
HistoryDialog::HistoryDialog(QWidget *parent) : QDialog(parent)
{
    ...
    m_searchBox->setPlaceholderText("Filter by title, platform, format, or status...");
    connect(m_searchBox, &QLineEdit::textChanged, this, &HistoryDialog::onSearchChanged);
...
{
    m_allEntries = loadHistory();
    populateTable(m_allEntries);
}

void HistoryDialog::populateTable(const QList<HistoryEntry> &list)
{
...
void HistoryDialog::onSearchChanged(const QString &text)
{
    ...
    for (const auto &e : m_allEntries) {
        if (e.title.contains(q, Qt::CaseInsensitive) ||
            e.platform.contains(q, Qt::CaseInsensitive) ||
            e.format.contains(q, Qt::CaseInsensitive) ||
            e.url.contains(q, Qt::CaseInsensitive) ||
            e.outputFile.contains(q, Qt::CaseInsensitive) ||
            (e.success ? QStringLiteral("done") : QStringLiteral("error"))
                .contains(q, Qt::CaseInsensitive))
        {
            filtered << e;
        }
    }
