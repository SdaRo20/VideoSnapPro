    ...
    : QMainWindow(parent)
{
    ...
    connect(m_speedTimer, &QTimer::timeout, this, [this]() {
        ...
        qint64 bps = m_queue->totalSpeedBytesPerSec();
        QString speedStr;
        if (bps <= 0) {
            speedStr = "";
        } else if (bps < 1024.0 * 1024) {
            speedStr = QString::number(bps / 1024.0, 'f', 1) + " KB/s";
        } else {
            speedStr = QString::number(bps / (1024.0 * 1024.0), 'f', 2) + " MB/s";
        }
        m_speedLabel->setText(QString("Active: %1 dl%2")
            .arg(m_queue->activeCount())
            .arg(speedStr.isEmpty() ? "" : "  •  " + speedStr + " total"));
    });
    m_speedTimer->start();
}
...
void MainWindow::setupMenuBar()
{
    ...
    auto *copyQueueAct = new QAction("Copy All Queued URLs", this);
    connect(copyQueueAct, &QAction::triggered, this, &MainWindow::onCopyAllUrls);
    toolsMenu->addAction(copyQueueAct);
    auto *importQueueAct = new QAction("Import Queue from File...", this);
    connect(importQueueAct, &QAction::triggered, this, &MainWindow::onImportQueue);
    toolsMenu->addAction(importQueueAct);
    auto *exportQueueAct = new QAction("Export Queue to File...", this);
    connect(exportQueueAct, &QAction::triggered, this, &MainWindow::onExportQueue);
    toolsMenu->addAction(exportQueueAct);
    ...
    auto *scraperAct = new QAction("Scraper", this);
    scraperAct->setShortcut(QKeySequence("Ctrl+S"));
    connect(scraperAct, &QAction::triggered, this, &MainWindow::onOpenScraper);
    toolsMenu->addAction(scraperAct);
...
void MainWindow::setupUi()
{
    ...
    // ── Input box ───────────────────────────────────────────────────────────
    auto *inputBox = new QGroupBox("Add URLs (YouTube, TikTok, Instagram, Facebook, Twitter/X, Vimeo — one per line)");
    ...
    auto *scraperBtn = new QPushButton("Scraper");
    scraperBtn->setFixedHeight(32);
    scraperBtn->setFixedWidth(80);
    scraperBtn->setToolTip("Scrape videos from a channel or profile (Ctrl+S)");
    connect(scraperBtn, &QPushButton::clicked, this, &MainWindow::onOpenScraper);
    ...
    optRow->addWidget(scraperBtn);
    optRow->addWidget(m_addBtn);
    optRow->addStretch();

    inputLayout->addLayout(optRow);

    // ── Queue controls row — playback + schedule + queue tools ───────────────
    auto *queueRow = new QHBoxLayout;
    queueRow->setSpacing(8);

    queueRow->addWidget(m_startBtn);
    queueRow->addWidget(m_pauseBtn);
    queueRow->addWidget(m_abortBtn);
    queueRow->addWidget(m_clearBtn);

    auto *sep = new QFrame;
    sep->setFrameShape(QFrame::VLine);
    sep->setFrameShadow(QFrame::Sunken);
    queueRow->addWidget(sep);
    ...
    queueRow->addWidget(schedLabel);
    queueRow->addWidget(m_scheduleEdit);
    queueRow->addWidget(m_scheduleBtn);
    queueRow->addWidget(m_scheduleLbl);

    queueRow->addStretch();

    auto *importBtn = new QPushButton("Import Queue");
    importBtn->setFixedHeight(32);
    importBtn->setToolTip("Load URLs from a text file into the input box");
    connect(importBtn, &QPushButton::clicked, this, &MainWindow::onImportQueue);
    queueRow->addWidget(importBtn);

    auto *exportBtn = new QPushButton("Export Queue");
    exportBtn->setFixedHeight(32);
    exportBtn->setToolTip("Save all queued URLs to a text file");
    connect(exportBtn, &QPushButton::clicked, this, &MainWindow::onExportQueue);
    queueRow->addWidget(exportBtn);

    inputLayout->addLayout(queueRow);
    root->addWidget(inputBox);

    // ── Download Table ───────────────────────────────────────────────────────
    m_table = new QTableWidget(0, COL_COUNT);
    m_table->setHorizontalHeaderLabels({
        "Thumb","Title","Platform","Format","Status","Progress","Speed","ETA","Size","Actions"
    });
    ...
    hdr->setSectionResizeMode(COL_ACTION,   QHeaderView::Fixed);        m_table->setColumnWidth(COL_ACTION, 92);
    ...
    connect(m_table, &QTableWidget::customContextMenuRequested, this,
            [this](const QPoint &pos) {
                ...
                menu.addSeparator();
                menu.addAction("Copy All Queued URLs", this, &MainWindow::onCopyAllUrls);
                menu.addAction("Import Queue from File...", this, &MainWindow::onImportQueue);
                menu.addAction("Export Queue to File...", this, &MainWindow::onExportQueue);
...
void MainWindow::setupTray()
{
    ...
    auto *showAct  = new QAction("Show Window", this);
    auto *startAct = new QAction("Start Downloads", this);
    ...
    connect(startAct, &QAction::triggered, this, &MainWindow::onStartAll);
    m_trayMenu->addAction(showAct);
    m_trayMenu->addAction(startAct);
...
{
    // Only minimize to tray if user explicitly enabled it in Settings
    if (m_settings.minimizeToTray && m_tray && m_tray->isVisible()) {
        hide();
        e->ignore();
    } else {
        saveWindowState();
...
{
    // Only intercept if user opted in to tray minimization
    if (e->type() == QEvent::WindowStateChange &&
        m_settings.minimizeToTray &&
        isMinimized())
    {
        QTimer::singleShot(50, this, &QWidget::hide);
    }
...
int MainWindow::addTableRow(const DownloadItem &item)
{
    ...
    // Action cell — pause/resume, retry, remove
    auto *actionWidget = new QWidget;
    auto *actionLayout = new QHBoxLayout(actionWidget);
    actionLayout->setContentsMargins(2, 2, 2, 2);
    actionLayout->setSpacing(2);

    auto mkActionBtn = [](const QString &text, const QString &color, const QString &tip) {
        ...
        b->setStyleSheet(QString(
            ...
            "QPushButton:hover { color: #ffffff; }").arg(color));
        return b;
    };

    auto *pauseBtn = mkActionBtn("⏸", "#d4a84b", "Pause this download");
    auto *retryBtn = mkActionBtn("⟲", "#5a9fd4", "Retry this download");
    auto *rmBtn    = mkActionBtn("✕", "#e04444", "Remove from queue");

    pauseBtn->setObjectName("rowPauseBtn");
    retryBtn->setObjectName("rowRetryBtn");
    rmBtn->setObjectName("rowRemoveBtn");

    retryBtn->setVisible(false); // shown only when item errors out

    int capturedId = item.id;

    connect(pauseBtn, &QPushButton::clicked, this, [this, capturedId, pauseBtn]() {
        if (m_queue->isItemPaused(capturedId)) {
            m_queue->resumeItem(capturedId);
            pauseBtn->setText("⏸");
            pauseBtn->setToolTip("Pause this download");
        } else {
            m_queue->pauseItem(capturedId);
            pauseBtn->setText("▶");
            pauseBtn->setToolTip("Resume this download");
        }
    });

    connect(retryBtn, &QPushButton::clicked, this, [this, capturedId, retryBtn, pauseBtn]() {
        if (!m_items.contains(capturedId)) return;
        retryBtn->setVisible(false);
        pauseBtn->setVisible(true);
        pauseBtn->setText("⏸");
        pauseBtn->setToolTip("Pause this download");
        DownloadItem &it = m_items[capturedId];
        it.retryCount = 0;
        m_queue->retryItem(it);
        ...
        if (r >= 0) {
            ...
            if (progItem) progItem->setData(Qt::DisplayRole, 0);
            --m_errorCount;
        }
        updateStatusBar();
    });
    ...
    actionLayout->addWidget(rmBtn);
    m_table->setCellWidget(row, COL_ACTION, actionWidget);
...
void MainWindow::onAddLinks()
{
    ...
    for (const QString &line : lines) {
        ...
        bool isProfile = PlatformDetector::isProfileUrl(url);
        bool isPlaylist = isProfile || PlatformDetector::isPlaylistUrl(url);

        // Normalize YouTube Shorts URL → regular watch URL
        // youtube.com/shorts/VIDEO_ID → youtube.com/watch?v=VIDEO_ID
        QString resolvedUrl = url;
        static QRegularExpression shortsRe(R"(youtube\.com/shorts/([A-Za-z0-9_-]+))");
        auto shortsMatch = shortsRe.match(url);
        if (shortsMatch.hasMatch()) {
            resolvedUrl = "https://www.youtube.com/watch?v=" + shortsMatch.captured(1);
            isPlaylist  = false;
        } else if (isProfile && url.contains("youtube.com/@") &&
                   !url.contains("/videos")) {
...
void MainWindow::onOpenScraper()
{
    ScraperDialog dlg(m_settings.savePath, m_settings.defaultFormat, this);

    if (dlg.exec() == QDialog::Accepted && dlg.hasDownloadRequest()) {
        auto req = dlg.downloadRequest();

        if (req.urls.isEmpty()) return;

        // Apply thread count from scraper
        if (req.threads > 0)
            m_queue->setMaxConcurrent(req.threads);

        // Update save path if changed in scraper
        if (!req.savePath.isEmpty() && req.savePath != m_settings.savePath) {
            m_settings.savePath = req.savePath;
            m_settings.save();
        }

        QString fmt  = req.format;
        QString path = req.savePath.isEmpty() ? m_settings.savePath : req.savePath;
        QDir().mkpath(path);

        int added = 0;
        for (const QString &url : req.urls) {
        ...
        statusBar()->showMessage(
            QString("Scraper: %1 video(s) added to queue. Click Start to download.")
                .arg(added), 5000);
        updateStatusBar();
    }
}
...
void MainWindow::onTrayActivated(QSystemTrayIcon::ActivationReason reason)
{
    if (reason == QSystemTrayIcon::DoubleClick ||
        reason == QSystemTrayIcon::Trigger) {   // single click also restores
...
{
    if (m_items.isEmpty()) {
        statusBar()->showMessage("Queue is empty — nothing to copy.", 3000);
        return;
    }

    QStringList urls;
    for (int r = 0; r < m_table->rowCount(); ++r) {
        auto *thumb = m_table->item(r, COL_THUMB);
        if (!thumb) continue;
        int id = thumb->data(ItemIdRole).toInt();
    ...
    statusBar()->showMessage(
        QString("Copied %1 URL(s) to clipboard.").arg(urls.size()), 3000);
}

void MainWindow::onExportQueue()
{
    if (m_items.isEmpty()) {
        QMessageBox::information(this, "Export Queue",
            "The queue is empty — nothing to export.");
        return;
    }

    QString path = QFileDialog::getSaveFileName(
        this, "Export Queue to File",
        QDir::homePath() + "/videosnap_queue.txt",
    ...
    for (int r = 0; r < m_table->rowCount(); ++r) {
        ...
        auto *statItem = m_table->item(r, COL_STATUS);
        QString status = statItem ? statItem->text() : "";

        lines << QString("%1\t[%2]\t%3")
                     .arg(item.url)
                     .arg(status)
                     .arg(item.title.isEmpty() ? "(no title)" : item.title);
    }

    QFile f(path);
    if (!f.open(QIODevice::WriteOnly | QIODevice::Text)) {
        QMessageBox::warning(this, "Export Queue",
    ...
    statusBar()->showMessage(
        QString("Exported %1 item(s) to %2").arg(lines.size()).arg(path), 5000);
}

void MainWindow::onImportQueue()
{
    QString path = QFileDialog::getOpenFileName(
        this, "Import Queue from File",
        QDir::homePath(),
        "Text Files (*.txt);;All Files (*)");
    if (path.isEmpty()) return;
    ...
    if (!f.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QMessageBox::warning(this, "Import Queue",
    ...
    while (!in.atEnd()) {
        ...
        // Accept plain URLs, or our own export format:
        //   "<url>\t[status]\t<title>"
        // — take just the first whitespace-delimited token if it's a URL.
        QString firstToken = line.split(QRegularExpression(R"(\s+)")).value(0);
    ...
    if (urls.isEmpty()) {
        QMessageBox::information(this, "Import Queue",
            "No valid URLs were found in that file.");
        return;
    }

    // Append to whatever is already in the input box rather than overwrite
    QString existing = m_urlInput->toPlainText().trimmed();
    QString combined = existing.isEmpty() ? urls.join("\n")
                                           : existing + "\n" + urls.join("\n");
    m_urlInput->setPlainText(combined);

    statusBar()->showMessage(
        QString("Imported %1 URL(s) from %2 — click 'Add to Queue' to enqueue them.")
            .arg(urls.size()).arg(QFileInfo(path).fileName()), 5000);
}
                                ...
                                const QString &outputFile)
{
    ...
    auto *spdItem  = m_table->item(r, COL_SPEED);
    auto *etaItem  = m_table->item(r, COL_ETA);
    auto *actionW  = m_table->cellWidget(r, COL_ACTION);
    auto *pauseBtn = actionW ? actionW->findChild<QPushButton*>("rowPauseBtn") : nullptr;
    auto *retryBtn = actionW ? actionW->findChild<QPushButton*>("rowRetryBtn") : nullptr;

    bool aborted = !success && errorMsg == "Aborted by user";

    // Auto-retry failed downloads (not user-initiated pauses/aborts) up to
    // DownloadItem::MAX_AUTO_RETRIES before surfacing an error to the user.
    if (!success && !aborted && m_items.contains(id) &&
        m_items[id].retryCount < DownloadItem::MAX_AUTO_RETRIES)
    {
        m_items[id].retryCount++;
        int attempt = m_items[id].retryCount;

        if (statItem) {
            statItem->setText(QString("Retrying (%1/%2)...")
                                   .arg(attempt).arg(DownloadItem::MAX_AUTO_RETRIES));
            statItem->setForeground(QColor("#d4a84b"));
        ...
        m_queue->retryItem(m_items[id]);
        updateStatusBar();
        return;
    }

    if (success) ++m_doneCount; else if (!aborted) ++m_errorCount;

    if (statItem) {
        statItem->setText(success ? "Done" : (aborted ? "Aborted" : "Error"));
        statItem->setForeground(success ? QColor(80,200,80)
                                : aborted ? QColor("#888")
                                          : QColor(220,60,60));
    ...
    // Show retry button on real failures; hide pause/resume once finished.
    if (pauseBtn) pauseBtn->setVisible(false);
    if (retryBtn) retryBtn->setVisible(!success && !aborted);
